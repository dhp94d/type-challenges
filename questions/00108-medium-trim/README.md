...
type ExcludeWord = ' '| '\n' | '\t';
type Trim<S extends string> = S extends `${ExcludeWord}${infer Rest}` | `${infer Rest}${ExcludeWord}` ? Trim<Rest> : S;
...
**해설**
ts4.1에 도입된 템플릿 리터럴타입을 알고 있어야 풀 수 있습니다.
템플릿 리터럴을통하여 해당 문자로 시작되면 재귀를돌려 leftTrim 기능을 구현하였습니다.
